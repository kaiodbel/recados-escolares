...
import React, { useReducer, useRef, useMemo, useEffect, useCallback, useContext, createContext, useState, useLayoutEffect } from 'react';
...
function createUseObject(useRealm) {
    function useObject(type, primaryKey, keyPaths) {
        ...
        const memoizedKeyPaths = useMemo(() => (typeof keyPaths === "string" ? [keyPaths] : keyPaths), 
...
})(AuthOperationName || (AuthOperationName = {}));
/**
...
const AuthOperationContext = createContext(null);
const AuthOperationProvider = ({ children }) => {
    const [authOpResult, setAuthOpResult] = useState({
...
function AppProviderWithConfiguration({ appRef, children, ...config }) {
    ...
    const [app, setApp] = useState(() => new Realm.App(configRef.current));
...
function createRealmProviderFromConfig(realmConfig, RealmContext) {
    return ({ children, fallback: Fallback, closeOnUnmount = true, realmRef, ...restProps }) => {
        ...
        const configuration = useRef(mergeRealmConfiguration(realmConfig, restProps));
...
function useAuth() {
    ...
    const logIn = useAuthOperation({
        operation: useCallback((credentials) => app.logIn(credentials), [app]),
    ...
    const logInWithApiKey = useAuthOperation({
        operation: useCallback((key) => app.logIn(Realm.Credentials.apiKey(key)), [app]),
        operationName: AuthOperationName.LogInWithApiKey,
    });
    const logInWithEmailPassword = useAuthOperation({
        operation: useCallback((params) => app.logIn(Realm.Credentials.emailPassword(params.email, params.password)), [app]),
        operationName: AuthOperationName.LogInWithEmailPassword,
    });
    const logInWithJWT = useAuthOperation({
        operation: useCallback((token) => app.logIn(Realm.Credentials.jwt(token)), [app]),
        operationName: AuthOperationName.LogInWithJWT,
    });
    const logInWithGoogle = useAuthOperation({
        operation: useCallback((credentials) => app.logIn(Realm.Credentials.google(credentials)), [app]),
        operationName: AuthOperationName.LogInWithGoogle,
    });
    const logInWithApple = useAuthOperation({
        operation: useCallback((idToken) => app.logIn(Realm.Credentials.apple(idToken)), [app]),
        operationName: AuthOperationName.LogInWithApple,
    });
    const logInWithFacebook = useAuthOperation({
        operation: useCallback((accessToken) => app.logIn(Realm.Credentials.facebook(accessToken)), [app]),
        operationName: AuthOperationName.LogInWithFacebook,
    });
    const logInWithFunction = useAuthOperation({
        operation: useCallback((payload) => app.logIn(Realm.Credentials.function(payload)), [app]),
...
function useEmailPasswordAuth() {
    ...
    const [result] = useAuthResult();
    const logIn = useAuthOperation({
        operation: useCallback((credentials) => app.logIn(Realm.Credentials.emailPassword(credentials)), [app]),
        operationName: AuthOperationName.LogIn,
    });
    const register = useAuthOperation({
        operation: useCallback((credentials) => app.emailPasswordAuth.registerUser(credentials), [app]),
        operationName: AuthOperationName.Register,
    });
    const confirm = useAuthOperation({
        operation: useCallback((args) => app.emailPasswordAuth.confirmUser(args), [app]),
        operationName: AuthOperationName.Confirm,
    });
    const resendConfirmationEmail = useAuthOperation({
        operation: useCallback((args) => app.emailPasswordAuth.resendConfirmationEmail(args), [app]),
        operationName: AuthOperationName.ResendConfirmationEmail,
    });
    const retryCustomConfirmation = useAuthOperation({
        operation: useCallback((args) => app.emailPasswordAuth.retryCustomConfirmation(args), [app]),
        operationName: AuthOperationName.RetryCustomConfirmation,
    });
    const sendResetPasswordEmail = useAuthOperation({
        operation: useCallback((args) => app.emailPasswordAuth.sendResetPasswordEmail(args), [app]),
        operationName: AuthOperationName.SendResetPasswordEmail,
    });
    const callResetPasswordFunction = useAuthOperation({
        operation: useCallback((credentials, ...restArgs) => app.emailPasswordAuth.callResetPasswordFunction(credentials, ...restArgs), [app]),
...
export { AppProvider, AuthError, AuthOperationName, OperationState, RealmProvider, UserProvider, createRealmContext, createUseObject, createUseQuery, useApp, useAuth, useAuthResult, useEmailPasswordAuth, useObject, useProgress, useQuery, useRealm, useUser };
